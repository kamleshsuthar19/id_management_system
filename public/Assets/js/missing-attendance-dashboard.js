...
async function loadWorkers() {
    try {
        const response = await fetch('/missing-attendance/records');
        ...
        console.error("Error loading missing attendance:", err);
        showEmptyState();
    }
}
...
// Render functions
function renderMissingAttendance() {
    showLoading();
...
function renderDesktopTable(attendance) {
    workerTableBody.innerHTML = attendance.map(record => `
<tr class="hover:bg-secondary-50 transition-fast">
    <td class="px-6 py-4 whitespace-nowrap text-sm text-text-secondary font-data text-center">${formatDate(record.attendance_date)}</td>
    <td class="px-6 py-4">
    <input type="checkbox" value="${safeText(record.userID)}" 
            onchange="handleRowSelection('${safeText(record.userID)}', this.checked)"
            class="rowCheckbox rounded border-secondary-300 text-primary focus:ring-primary-500">
    </td>
    <td class="px-6 py-4 whitespace-nowrap">
    <span class="text-sm font-medium text-text-primary font-data">${safeText(record.userID)}</span>
    </td>
    ...
    <div class="flex items-center">
        ...
        <div class="h-8 w-8 rounded-full bg-primary-100 flex items-center justify-center">
            <span class="text-xs font-medium text-primary">${safeFirstChar(record.present)}</span>
        </div>
        </div>
        <div class="ml-3">
        <div class="text-sm font-medium text-text-primary">${safeText(record.present)}</div>
        </div>
    </div>
    </td>

    <td class="px-6 py-4 whitespace-nowrap text-sm text-text-secondary text-center">${record.in_time}</td>
    <td class="px-6 py-4 whitespace-nowrap text-sm text-text-secondary">${record.out_time}</td>
    <td class="px-6 py-4 whitespace-nowrap text-sm text-text-secondary font-data text-center">${record.remarks}</td>
    <td class="px-6 py-4 whitespace-nowrap text-sm text-text-secondary font-data text-center">${record.remarks}</td>
    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
        <div class="flex items-center space-x-2">
            <button onclick="exportAttendance('${worker.userID}')" 
